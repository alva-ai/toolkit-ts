...
describe('known-component-class', () => {
  it('passes for registered classes', () => {
    const m = buildModel(parseHtml('<button class="btn btn-primary btn-large">X</button>'), CONTRACT);
    expect(knownComponentClass(m, CONTRACT)).toEqual([]);
  });

  it('errors on unregistered btn-* class', () => {
    const m = buildModel(parseHtml('<button class="btn btn-huge">X</button>'), CONTRACT);
  ...
  it('does not flag unrelated classes', () => {
    const m = buildModel(parseHtml('<div class="my-custom-thing">X</div>'), CONTRACT);
    expect(knownComponentClass(m, CONTRACT)).toEqual([]);
  });
});
